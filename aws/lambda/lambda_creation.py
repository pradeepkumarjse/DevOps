...
source_access_key = ''
source_secret_key = ''
account_id = '254438258404'  # Replace with your AWS account ID
...
def create_lambda_function(lambda_client, function_details):
    config = function_details['Configuration']
        # Fetch the existing function configuration to get layers
    try:
        existing_function_config = lambda_client_source.get_function_configuration(
            FunctionName=config['FunctionName']
        )
        existing_layers = existing_function_config.get('Layers', [])
        layer_arns =  [layer['Arn'].replace(source_region, target_region) for layer in existing_layers]
    except Exception as e:
        print(f"Failed to retrieve layers from source function: {str(e)}")
        layer_arns = []
    print(layer_arns)
    code_config = {}
...
def lambda_handler(event, context):
    ...
    for function in functions:
        try:
            ...
            print(f"Failed to create function {function_name} in target account.")
    return {
        'statusCode': 200,
